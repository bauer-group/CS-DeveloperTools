...
PATTERN_PRESETS = {
    "claude": [
        r"🤖\s*Generated with\s*\[Claude Code\]\(https://claude\.com/claude-code\)\s*",
...
def rewrite_history(repo: Repo, branch: str, patterns: List[str], dry_run: bool = False) -> int:
    ...
    try:
        for commit in commits:
            ...
            if has_match:
                rewritten += 1
                print(f"{CYAN}→{NC} {commit.hexsha[:8]}: pattern removed")
...
Examples:
  ...
  git-rewrite-commits.py -p "TICKET-\\d+:\\s*" --dry-run
    ...
    print(f"{BOLD}{CYAN}║                  Git Commit Rewriter                          ║{NC}")
    print(f"{BOLD}{CYAN}╚═══════════════════════════════════════════════════════════════╝{NC}")
    print()
    ...
    if args.analyze:
        ...
        print()
        print(f"{GREEN}✓ {count} commits rewritten{NC}")
