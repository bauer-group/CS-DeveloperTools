...
Examples:
  ...
  gh-actions-usage.py --json
    ...
    # Get billing info
    if not args.json:
        print(f"Fetching billing data for {BOLD}{args.org}{NC}...")

    billing = get_org_billing(args.org)

    if args.json:
        output_data = {"org": args.org, "billing": billing, "repos": []}
    ...
    if billing:
        ...
        if not args.json:
            print()
            print(f"{BOLD}Organization Billing Summary:{NC}")
            print(f"  Total minutes used: {CYAN}{total_minutes:,}{NC}")
            print(f"  Included minutes: {included_minutes:,}")
            print(f"  Paid minutes: {YELLOW}{paid_minutes:,}{NC}")
            print()
    ...
    if not args.detailed and not args.repo:
        print(f"Use {BOLD}--detailed{NC} for per-repo breakdown")
        print()
